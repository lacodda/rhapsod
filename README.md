...
```json
{"status":"ok","version":"0.13.1","pieces":2,"indexed_seconds_ago":1450}
```
...
**And now it is ready for the road, and says so.** The stand screen opens with the one question worth asking before leaving the house - whether the library will be there on the train - and answers it in a sentence, with the reason and the one thing to fix. It is a real answer: a page served over plain HTTP cannot keep an offline copy however long it is left open, and until now nothing said so. Which shelves ride along is now a choice - a library of three hundred pieces is not all wanted in a bag - and the count, the verdict and what the device fetches all mean the same selection. The plan of what could be written is carried too, so a novella can be asked for from a train rather than only remembered until you are home. The reader installs to a home screen from the app itself where the browser allows it, and says how where it does not. Behind all of it, a new gate runs a real browser against a real build before every merge: the worker registers, the cache fills, the stand is switched off and the library still opens. The offline reader broke quietly once; it cannot now.
